...
const Genres = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-100 flex flex-col items-center py-10">
      ...
      <div className="flex items-center bg-white shadow-md rounded-full px-4 py-2 mb-10 w-80">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredGenres.map((genre, index) => (
          <motion.div
            ...
            whileTap={{ scale: 0.95 }}
            className="bg-white shadow-xl rounded-2xl p-6 flex flex-col items-center justify-center transition-all hover:shadow-2xl"
          >
            {genre.icon}
      ...
      {filteredGenres.length === 0 && (
        <p className="mt-8 text-gray-500 italic">No genres found 😢</p>
      )}
    </div>
  );
};
